...
const SUGGESTIONS = ['Coffee shop landing page', 'SaaS pricing page', 'Portfolio site'];
  ...
  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column', gap:'6px', padding:'0 10px 10px', overflow:'hidden' }}>
      ...
      <button onClick={handleSubmit} disabled={isDisabled}
        style={{ padding:'7px', borderRadius:'4px', border:'none', background: isDisabled ? '#2d2d2d' : '#0078d4', color: isDisabled ? '#555' : '#fff', fontSize:'11px', fontWeight:700, cursor: isDisabled ? 'not-allowed' : 'pointer', transition:'all 0.15s' }}
      >
