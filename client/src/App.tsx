...
const AccountPage = lazy(() => import("./features/account/components/AccountPage"));
...
function LandingPage() {
  ...
  // ← Tera kaam — localStorage mein save karke editor pe bhejo
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col selection:bg-indigo-500 selection:text-white relative overflow-hidden">
      ...
      <header className="sticky top-0 z-50 backdrop-blur-md bg-slate-950/70 border-b border-slate-800/80 transition-all duration-300">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <a href="https://github.com" target="_blank" rel="noreferrer" className="p-2 rounded-full hover:bg-slate-800/60 text-slate-400 hover:text-white transition-colors duration-200">
      ...
      <main className="flex-1 max-w-7xl mx-auto px-6 py-12 w-full flex flex-col justify-center relative z-10">
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <div className="flex flex-col md:flex-row items-center justify-center gap-8">
            {/* Edit in Editor button — sirf generate hone ke baad dikhega */}
            {generatedHtml && (
              <button
                onClick={handleOpenEditor}
                className="px-6 py-3 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 font-semibold text-white shadow-lg shadow-indigo-600/30 transition-all duration-300 cursor-pointer flex items-center gap-2 group"
              >
                ✏️ Edit in Editor
          ...
          {generatedHtml && (
            <div className="max-w-7xl mx-auto mb-16 mt-8">
              <h2 className="text-3xl font-bold mb-6 text-center">Website Preview</h2>
              <div className="flex justify-center gap-3 mb-6">
                <button onClick={() => setActiveTab("preview")} className={`px-4 py-2 rounded-lg ${activeTab === "preview" ? "bg-indigo-600 text-white" : "bg-slate-800 text-slate-300"}`}>Preview</button>
                <button onClick={() => setActiveTab("code")} className={`px-4 py-2 rounded-lg ${activeTab === "code" ? "bg-indigo-600 text-white" : "bg-slate-800 text-slate-300"}`}>Code</button>
              </div>
              ...
              {activeTab === "preview" ? (
                <iframe title="Website Preview" srcDoc={generatedHtml} className="w-full h-[900px] rounded-2xl border border-slate-700 bg-white shadow-2xl" sandbox="allow-scripts" />
              ) : (
          ...
          <div className="max-w-5xl mx-auto mb-16">
            <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-6 backdrop-blur-sm">
              <h2 className="text-xl font-bold mb-4 text-cyan-400">Generation Activity</h2>
              <div className="min-h-[120px] text-slate-300 whitespace-pre-wrap leading-relaxed">
                {streamData ? <div dangerouslySetInnerHTML={{ __html: streamData }} /> : "AI response will appear here..."}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-5xl mx-auto mb-16">
          <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-6">
            <h2 className="text-2xl font-bold mb-6 text-indigo-400">Project History</h2>
            <div className="space-y-3">
              {projects.map((project) => (
                ...
                >
                  <div className="font-semibold text-white">{project.prompt}</div>
                  <div className="text-sm text-slate-400">{project.created_at}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
          <div className="p-6 rounded-2xl border border-slate-800/80 bg-slate-900/40 backdrop-blur-sm relative overflow-hidden group hover:border-slate-700/80 transition-all duration-300">
            ...
            <p className="text-sm text-slate-400 leading-relaxed mb-4">Vite dev server running on port 3000.</p>
            <div className="text-xs font-mono text-indigo-400/90 bg-indigo-950/40 px-3 py-2 rounded-lg border border-indigo-900/30">cd client && npm run dev</div>
          </div>
          <div className="p-6 rounded-2xl border border-slate-800/80 bg-slate-900/40 backdrop-blur-sm relative overflow-hidden group hover:border-slate-700/80 transition-all duration-300">
            ...
            <p className="text-sm text-slate-400 leading-relaxed mb-4">FastAPI with AWS Bedrock and Amazon Nova Pro.</p>
            <div className="text-xs font-mono text-purple-400/90 bg-purple-950/40 px-3 py-2 rounded-lg border border-purple-900/30">cd server && uvicorn app.main:app</div>
          </div>
          <div className="p-6 rounded-2xl border border-slate-800/80 bg-slate-900/40 backdrop-blur-sm relative overflow-hidden group hover:border-slate-700/80 transition-all duration-300">
      ...
      <footer className="border-t border-slate-900 bg-slate-950/40 py-8 relative z-10">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-xs text-slate-500">&copy; {new Date().getFullYear()} Capstone Shery. All rights reserved.</p>
  ...
  return (
    <BrowserRouter>
      <Suspense fallback={<PageLoader />}>
        <Routes>
          {/* Landing page — Member B ka kaam */}
          <Route path="/" element={<LandingPage />} />

          {/* Editor — tera kaam (Member C) */}
          <Route path="/editor/:projectId" element={
            <Suspense fallback={<PageLoader />}>
              <lazy(() => import("@/features/editor/components/EditorLayout")) />
            </Suspense>
          } />
